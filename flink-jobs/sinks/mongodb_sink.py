"""
MongoDB Sink Connector for Apache Flink
Writes processed data from Flink to MongoDB
"""
import logging
...
class MongoDBSink:
    ...
    def close(self):
        ...
        try:
            self.flush()  # Flush remaining documents
            if self.client:
                self.client.close()
            logger.info(
...
class MongoDBBatchSink:
    """
    Batch-optimized MongoDB sink for high-throughput scenarios
    """
    ...
    def __init__(self, connection_url: str, database: str, collections: Dict[str, str]):
        ...
        Args:
            connection_url: MongoDB connection URL
            database: Database name
            collections: Dict mapping data types to collection names
        """
        self.connection_url = connection_url
        self.database_name = database
    ...
    def write(self, data_type: str, document: Dict[str, Any]):
        ...
        Args:
            data_type: Type of data (determines collection)
            document: Document to write
        """
